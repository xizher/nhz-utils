...
;(async function () {
  ...
  names.forEach(async name => {
    ...
    if (isDir) {
      await execCmd(/^win/.test(process.platform) ? 'npx.cmd' : 'npx', ['typedoc', '--out', `src/${name}`, `src/${name}/${name}.ts`, '--cleanOutputDir', 'false', '--githubPages', 'false', '--readme', 'none', '--name', name])
    }
  })
})()
